...
MLEVar <- 1 / 2
k <- 1.5
plot(delta, (delta - 1)^2 * k^2 + delta^2 * MLEVar,
  type = "l", ylim = c(0, 2.3),
...
legend("topright", c(
  "prior bias = 1.5", "prior bias =  1",
  "prior bias = 1/2"
), lty = c(1, 2, 3), lwd = 2, col = c("black", "red", "blue"))
abline(h = MLEVar)
graphics.off()


## Examples 20.9 and 20.11 and Figure 20.4 ----
library(rjags)
library("Ecdat")
data(SP500)
r <- SP500$r500
...
inits <- function() {
  list(
    mu = rnorm(1, mean = mean(r), sd = 2 * sd(r)),
    tau = runif(1, 0.2 / var(r), 2 / var(r)), k = runif(1, 2.5, 10)
  )
}
t1 <- proc.time()
univt.mcmc <- jags.model("univt.bug",
...
pdf("basic_plot.pdf", width = 5, height = 6)
par(mfrow = c(3, 2))
...
pdf("gelman_plot.pdf", width = 6, height = 6)
...
inits <- function() {
  list(
    beta = as.numeric(rmvnorm(n = 1, mean = betahat, sigma = 1.5 * covbetahat)),
    tau = runif(1, 1 / (4 * summ$sigma^2), 4 / summ$sigma^2)
  )
}
regr <- jags.model("lin_reg_vect.bug",
  data = data, inits = inits,
  n.chains = 3, n.adapt = 1000, quiet = FALSE
)
...
regr.noco <- jags.model("lin_reg_vect.bug",
  data = data, inits = inits,
  n.chains = 3, n.adapt = 1000, quiet = FALSE
)
...
## Example 20.11 with the code for Example 20.9 ----
...
inits.midCap <- function() {
  list(alpha = 0.001, mu = means, tau_eps = tau_eps, tau_mu = tau_mu)
}
midCap <- jags.model("midCap.bug",
  data = data, inits = inits.midCap,
  n.chains = 3, n.adapt = 1000, quiet = FALSE
)
nthin <- 20
midCap.coda <- coda.samples(midCap, c(
  "mu", "tau_mu", "tau_eps", "alpha",
  "sigma_mu", "sigma_eps"
), 500 * nthin, thin = nthin)
summ.midCap <- summary(midCap.coda)
summ.midCap
post.means <- summ.midCap[[1]][2:21, 1]
delta <- 5.4 / (5.4 + 75.7)
print(delta, digits = 2)
pdf("midcap.pdf", width = 6, height = 3.75)
par(mfrow = c(1, 2))
...
axis(1, labels = F, tick = T, lwd.tick = 0)
for (i in 1:m) {
  lines(1:2, c(post.means[i], mu2[i]), col = i)
}
graphics.off()
...
df_prior <- 6
data <- list(
  y = y, N = N, Prec_mu = Prec_mu,
  Prec_tau = Prec_tau,
  mu0 = mu0, m = m, df_likelihood = df_likelihood,
  df_prior = df_prior, df_wishart = df_wishart
)
inits_t_CRSP <- function() {
  list(
    mu = mvrnorm(1, mu0, diag(rep(1, m) / 100)),
    tau = rwish(6, diag(rep(1, m)) / 100)
  )
}
library(R2WinBUGS)
multi_t.sim <- bugs(data, inits_t_CRSP,
...
library(rjags)
multi_t_CRSP <- jags.model("mult_t_CRSP.bug",
  data = data, inits = inits_t_CRSP,
  n.chains = 3, n.adapt = 1000, quiet = FALSE
)
nthin <- 1
...
N <- length(y)
data <- list(y = y, N = N)
inits_stochVol_ARMA11 <- function() {
  list(
    mu = rnorm(1, mean = mean(y), sd = sd(y) / sqrt(N)),
...
stochVol_ARMA11 <- jags.model("stochVol_ARMA11.bug",
  data = data, inits = inits_stochVol_ARMA11,
  n.chains = 3, n.adapt = 1000, quiet = FALSE
)
...
pdf("stochVolandGARChCondVar.pdf", width = 6, height = 5)
par(mfrow = c(1, 1))
plot(year, sqrt(h_stochVolARMA11),
  ylim = c(0, .035), ylab = "conditional std. dev.",
  type = "l", lwd = 2
)
...
pdf("stochVolandGARCH_acf.pdf", width = 8, height = 3.25)
par(mfrow = c(1, 3))
...
# alpha0 initiation is different here from in the book.
# Large values of alpha0 were causing problems.
inits_garch11 <- function() {
  list(
    alpha0 = runif(1, 0.001, 0.01),
    beta0 = runif(1, 0.001, 0.25), mu = runif(1, 0.001, 0.25),
    alpha1 = runif(1, 0.001, 0.25), nu = runif(1, 2, 10)
  )
}
t1 <- proc.time()
garch11 <- jags.model("garch11.bug",
  data = data, inits = inits_garch11,
  n.chains = 3, n.adapt = 1000, quiet = FALSE
)
nthin <- 20
...
Capm.jags <- jags.model("BayesCapm.bug",
  data = data, inits = inits.Capm,
  n.chains = 1, n.adapt = 1000, quiet = FALSE
)
nthin <- 10
N <- 500
Capm.coda <- coda.samples(Capm.jags,
  c("beta", "tauepsilon", "taubeta"), N * nthin,
  thin = nthin
)
...
pdf("two_betas.pdf", width = 6, height = 5)
lmFit <- lm(as.matrix(rtrain) ~ mkt_train)
beta_lm <- lmFit$coeff[2, ]
par(mfrow = c(1, 1))
plot(beta_lm, beta,
  xlim = c(0.7, 1.55), ylim = c(0.7, 1.55),
...
library(quadprog)
opt1 <- solve.QP(
  Dmat = as.matrix(lambda^2 * Omega), dvec = lambda * mu,
...
pdf("two_w_lambda3.pdf", width = 6, height = 5)
plot(w_model_free,
  type = "b", col = "blue", main = paste("lambda = ", lambda),
  ylab = "w", ylim = c(-2.25, 2.25), lwd = 2
)
points(w_Capm, type = "b", col = "red", pch = "*", lwd = 2)
legend("bottomleft", c("model-free", "CAPM"),
  lty = 1,
...
inits <- function() {
  list(
    mu = rnorm(1, 0, 0.3), tau = runif(1, 1, 10),
    k = runif(1, 1, 30)
  )
}
univ_t <- jags.model("univt.bug",
  data = ibm_data, inits = inits,
  n.chains = 3, n.adapt = 1000, quiet = FALSE
)
nthin <- 2
...
inits <- function() {
  list(
    mu = rnorm(1, 0, 2 * sd(y) / sqrt(N)),
    phi = rnorm(1, 0, 0.3), tau = runif(1, 1, 10)
  )
}
ar1 <- jags.model("ar1.bug",
  data = GDP_data, inits = inits,
  n.chains = 3, n.adapt = 1000, quiet = FALSE
)
nthin <- 20
...
inits.ma <- function() {
  list(
    mu = rnorm(1, mean(y), 2 * sd(y) / sqrt(N)),
    theta = rnorm(1, -0.05, 0.1), tau = runif(1, 5, 8)
  )
}

ma1 <- jags.model("ma1.bug",
  data = ma.sim_data, inits = inits.ma,
  n.chains = 3, n.adapt = 1000, quiet = FALSE
)
nthin <- 5
...
inits.arma11 <- function() {
  list(phi = rnorm(1, 0, 0.3), theta = rnorm(1, -0.5, 0.1), tau = runif(1, 5, 8))
}

arma11 <- jags.model("arma11.bug",
  data = arma11.sim_data, inits = inits.arma11,
  n.chains = 3, n.adapt = 1000, quiet = FALSE
)
nthin <- 5
...
tsReg.jags <- jags.model("tsReg.bug.R",
  data = data, inits = inits,
  n.chains = 1, n.adapt = 1000, quiet = FALSE
)
nthin <- 10
