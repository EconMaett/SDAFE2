...
par(mfrow = c(2, 1))
plot(y,
  ylab = "Inflation Rate", type = "l", xlab = "Year", cex.lab = 1.5,
  cex.axis = 1.5, cex.main = 1.3, main = "(a)"
)
plot(diff(y),
  ylab = "Change in Rate", type = "l", xlab = "Year", cex.lab = 1.5,
...
plot(x1,
  type = "l", xlab = "Time (t)", ylab = expression(Y[t]),
...
par(mfrow = c(1, 1))
plot(0:10, x1,
  xlab = "lag", ylab = "ACF", main = "ACF of three AR(2) processes", cex.axis = 1.5,
...
plot(resultsdiff[, 1], resultsdiff[, 2],
  xlab = "p", ylab = "criterion", cex.lab = 1.35, cex.axis = 1.35,
...
par(mfrow = c(2, 2))
plot(ts(y, start = 1950, frequency = 12),
  ylab = "Inflation Rate", type = "l", xlab = "Year", cex.lab = 1.5,
  cex.axis = 1.5, cex.main = 1.3, main = "(a)"
)
acf(y, main = "(b)")
fitAR7 <- arima(y, c(7, 0, 0))
plot(ts(fitAR7$resid, start = c(1950, 2), frequency = 12),
  ylab = "Inflation Rate", type = "l", xlab = "Year", cex.lab = 1.5,
...
par(mfrow = c(1, 1))
plot(resultsdiff[, 1], resultsdiff[, 2],
  xlab = "q", ylab = "criterion", cex.lab = 1.35, cex.axis = 1.35,
...
qqnorm(bestfit$resid, datax = T, main = "(b)")
qqline(bestfit$resid, datax = T)
...
plot(year[t1], y[t1],
  ylim = c(-10, 18), type = "b", xlim = c(1975, 1999),
...
pred.infl_diff <- predict(fit_diff,
  n.ahead = 100, newxreg = NULL,
  se.fit = TRUE
)
t1 <- 300:491
t2 <- 492:(492 + 49 + 50)
...
plot(year[t1], diff(y)[t1],
  xlim = c(1975, 1999), ylim = c(-9, 15), type = "b",
...
R <- Quandl("FRED/DTB3", start_date = "1955-01-01", end_date = "2013-12-31", collapse = "quarterly", sort = "asc")
r <- R$Value

# Billions of Chained 2005 Dollars Seasonally Adjusted Annual Rate,
# Source: US Federal Reserve Board
Y <- Quandl("FRED/GDPC96", start_date = "1955-01-01", end_date = "2013-12-31", collapse = "quarterly", sort = "asc")
y <- log(Y$Value)
...
PI <- Quandl("FRED/CPIAUCNS", start_date = "1954-10-01", end_date = "2013-12-31", collapse = "quarterly", sort = "asc")
