...
pdf("normalVaR.pdf", width = 6, height = 5) 
plot(alpha, var,
...
pdf("SPreturns_tplot.pdf", width = 6, height = 5)
par(mfrow = c(1, 1))
qqplot(SPreturn, qt(grid, df = 2.9837), main = "t-probability plot,
   df=2.9837", xlab = "data", ylab = "t-quantiles")
abline(lm(qt(c(0.25, 0.75), df = 2.9837) ~ quantile(SPreturn, c(0.25, 0.75))))
graphics.off()


## Example 19.4 and Figure 19.3 ----
data(SP500, package = "Ecdat")
library("fGarch")
n <- 2783
alpha <- 0.05
...
for (i in (1:B)) {
  ...
  q_b <- as.numeric(quantile(returns_b, 0.05))
...
write.table(VaRs, file = "SP500VaRBoot.txt", row.names = FALSE)

quantile(VaRs[, 1], c(0.025, 0.975))
quantile(VaRs[, 2], c(0.025, 0.975))
quantile(VaRs[, 3], c(0.025, 0.975))
quantile(VaRs[, 4], c(0.025, 0.975))
...
pdf("SP500_GARCH_CondSD.pdf", width = 6, height = 5)
...
## Example 19.7 and Figures 19.4, 19.5, and 19.6 ----
data(SP500, package = "Ecdat")
library("fGarch")
# daily observations from 1981?01 to 1991?04
# number of observations : 2783
...
pdf("SP500_tail_plots.pdf", width = 6, height = 5)
par(mfrow = c(2, 2))
m <- 50
xx <- log((1:m) / n)
yy <- log(-x[1:m])
...
sd <- param[2] * sqrt((df) / (df - 2))
lambda <- param[2]
alpha <- seq(0.002, 0.2, 0.0001)
qalpha <- qt(alpha, df = df)
VaR_par <- -20000 * (mean + lambda * qalpha)
VaR_norm <- -20000 * (mean(x) + sd(x) * qnorm(alpha))
pdf("var_sp500_est.pdf", width = 6, height = 5)
par(mfrow = c(1, 1))
plot(alpha, -20000 * q * (0.1 / alpha)^(1 / a),
  type = "l", lwd = 2,
  xlab = expression(alpha), ylab = expression(paste("VaR(", alpha, ")")),
  ylim = c(0, 1700), log = "x"
)
a.hill <- 2.2
lines(alpha, -20000 * q * (0.1 / alpha)^(1 / a.hill), lty = 1, lwd = 2, col = "red")
...
c <- seq(quantile(SPreturn, 0.025), quantile(SPreturn, 0.25), by = 0.001)
nc <- 0 * c
den <- nc
hill <- nc
...
pdf("var_sp500_hill.pdf", width = 5.25, height = 2.25)
par(mfrow = c(1, 3))
plot(nc, hill, type = "b", main = "(a)", ylab = "Hill estimator")
plot(nc, hill,
  xlim = c(25, 120), ylim = c(2, 2.4), type = "b", main = "(b)", ylab =
    "Hill estimator"
)
plot(nc, hill,
  xlim = c(60, 100), ylim = c(2, 2.4), type = "b", main = "(c)", ylab =
    "Hill estimator"
)
hill[8:11]
graphics.off()
...
## Example 19.8 and Figure 19.7 ----
pdf("nearlyDiscrete.pdf", width = 6, height = 3.5)
par(mfrow = c(1, 2))
sigma <- 2
sigma2 <- sqrt(2)
x <- seq(-120, 2005, length = 2000)
y <- .04 * pnorm(x, mean = 2000, sd = sigma) + .96 * pnorm(x, mean = -100, sd = sigma)
plot(x, y, type = "l", ylim = c(0.9, 1), xlab = "loss", ylab = "prob", main = "Portfolio 1")
abline(h = 0.95, lty = 2)
xspline <- seq(-120, -90, length = 2000)
yspline <- .04 * pnorm(xspline, mean = 2000, sd = sigma) + .96 * pnorm(xspline, mean = -100, sd = sigma)
options(digits = 5)
spline(yspline, xspline, xout = 0.95)
y1 <- (0.04^2) * pnorm(x, mean = 2000, sd = sigma2)
y2 <- (2 * .04 * .96) * pnorm(x, mean = 950, sd = sigma2)
y3 <- (0.96^2) * pnorm(x, mean = -100, sd = sigma2)
y <- y1 + y2 + y3
plot(x, y, type = "l", ylim = c(0.9, 1), xlab = "loss", ylab = "prob", main = "Portfolio 2")
abline(h = 0.95, lty = 2)
graphics.off()


## R lab ----
CokePepsi <- read.table("CokePepsi.csv", header= TRUE)
price <- CokePepsi[, 1]
