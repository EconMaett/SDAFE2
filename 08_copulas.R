...
plot(rho, lambda1,
  type = "l", lty = 1, xlab = expression(rho),
...
legend("topleft", c(
  expression(nu == 1), expression(nu == 4), expression(nu == 25),
  expression(nu == 250)
), lty = 1:4)
...
plot(fhatU,
  drawpoints = FALSE, drawlabels = FALSE, cont = seq(10, 80, 10),
...
par(mfrow = c(2, 2), cex.axis = 1.2, cex.lab = 1.2, cex.main = 1.2)
qqnorm(z1, datax = TRUE, main = "(a)")
qqline(z1)
qqnorm(z2, datax = TRUE, main = "(b)")
qqline(z2)
plot(z1, z2, main = "(c)", xlab = expression(hat(Z)[1]), ylab = expression(hat(Z)[2]), mgp = c(2.5, 1, 0))
plot(fhatZ,
  drawpoints = FALSE, drawlabels = FALSE, cont = seq(10, 90, 10),
...
plot(u1, u2,
  main = "Uniform-Transformed Data",
  xlab = expression(hat(U)[1]), ylab = expression(hat(U)[2])
)
Udex <- (1:n) / (n + 1)
Cn <- C.n(
  u = cbind(rep(Udex, n), rep(Udex, each = n)), U = U.hat,
  offset = 0, method = "C"
)
EmpCop <- expression(contour(Udex, Udex, matrix(Cn, n, n), col = 2, add = T))

contour(normalCopula(param = 0, dim = 2), pCopula,
  main = expression(C[0]),
  xlab = expression(hat(U)[1]), ylab = expression(hat(U)[2])
)
eval(EmpCop)
...
contour(
  tCopula(
    param = Ct@estimate[1], dim = 2,
    df = round(Ct@estimate[2])
  ),
  pCopula,
  main = expression(hat(C)[t]),
  xlab = expression(hat(U)[1]), ylab = expression(hat(U)[2])
)
eval(EmpCop)
...
cop_normal_dim3 <- normalCopula(
  dim = 3, param = c(-0.6, 0.75, 0),
  dispstr = "un"
)
mvdc_normal <- mvdc(
  copula = cop_normal_dim3, margins = rep("exp", 3),
  paramMargins = list(
    list(rate = 2), list(rate = 3),
    list(rate = 4)
  )
)
set.seed(5640)
...
netReturns = ((diff(IBM.SP500)/lag(IBM.SP500)*100)[-1,])
tail(netReturns)
colnames(netReturns) = c("IBM", "SP500")
colnames(netReturns)
head(netReturns)
tail(netReturns)
...
data1 <- cbind(
  pstd(ibm, est.ibm[1], est.ibm[2], est.ibm[3]),
...
ffrank <- fitCopula(
  copula = frankCopula(3, dim = 2),
  data = data1, method = "ml"
)
fclayton <- fitCopula(
  copula = claytonCopula(1, dim = 2),
  data = data1, method = "ml"
)
fgumbel <- fitCopula(
