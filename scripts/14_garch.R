...
lwdfact <- 6
par(mfrow = c(2, 2))
plot(xts(abs(SPreturn), date_SP),
  main = "(a) S&P 500 daily return", xlab = "year", type = "l",
  cex.axis = 1.5, cex.lab = 1.5, cex.main = 1.5, ylab = "|log return|", ylim = c(0, 0.08),
  minor.ticks = FALSE, major.ticks = "year", major.format = "%Y"
)
mod <- loess(abs(SPreturn) ~ year_SP, span = 0.25)
lines(xts(predict(mod), date_SP), lwd = lwdfact, col = 2)

plot(xts(abs(diffbp), date_dm),
...
plot(xts(abs(difflogrf), date_rf),
  main = "(c) Risk-free interest rate", xlab = "year",
...
plot(xts(abs(infl - mean(infl)), date_infl),
  ylab = "|rate - mean(rate)|", xlab = "year",
...
plot(sqrt(sig2[10001:n]),
  type = "l", xlab = "t", ylab = expression(sigma[t]),
...
a1 <- .1
b1 <- uniroot(f = function(b) {
  rho(a1, b)[2] - target
}, interval = c(0, 1 - a1))$root

a2 <- .3
b2 <- uniroot(f = function(b) {
  rho(a2, b)[2] - target
}, interval = c(0, 1 - a2))$root

a3 <- .5
b3 <- uniroot(f = function(b) {
  rho(a3, b)[2] - target
}, interval = c(0, 1 - a3))$root

pdf("garch11ACF.pdf", width = 6, height = 5)

plot(0:10, rho(a1, b1),
  type = "b", ylim = c(0, 1), lty = 1, lwd = 2,
...
legend("topright", c(
  expression(paste(alpha, " = 0.10, ", beta, " = 0.894")),
...
## Figure 14.8 ----
x <- seq(-3, 3, 0.005)
gamma <- c(-0.5, -0.2, 0, 0.12, 0.3, 0.9)
...
lwd1 <- 4
plot(BMW, type = "l", xlab = "year", ylab = "log return", ylim = c(-0.13, 0.21), xlim = c(date[3393], date[5219]), main = "Forecasting BMW returns")
# ,xlim=c(1986,1992),ylim=c(-0.13,.21),
...
plot(
  ts(EWMA.Sigma[1, 2, ] / (sqrt(EWMA.Sigma[1, 1, ] * EWMA.Sigma[2, 2, ])),
...
points(
  ts(mvwindow.cor(A[, 1], A[, 2], win = 126)$correlation,
    start = c(1989, 1), frequency = 253
  ),
...
plot(
  ts(
    DOC.fit$Sigma.pca[2, 1, ] / (sqrt(DOC.fit$Sigma.pca[1, 1, ] *
...
plot(
  ts(
    DOC.fit$Sigma.doc[2, 1, ] / (sqrt(DOC.fit$Sigma.doc[1, 1, ] *
