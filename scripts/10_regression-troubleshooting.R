...
pdf("leverage_ex.pdf", width = 6, height = 5)
...
pdf("leverage_ex_lev.pdf", width = 6, height = 5)
...
plot(x2, hatvalues(lm(y ~ x2)),
  xlab = "x", ylab = "leverage",
  main = "(d)", ylim = c(0, 1)
)
graphics.off()

pdf("leverage_ex_residualsNEW.pdf", width = 6, height = 5)
...
pdf("leverage_ex_cookD.pdf", width = 6, height = 5)
...
halfnorm(sqrt(cooks.distance(lm(y ~ x))),
  ylab = ("square root Cook's D"), cex = 1, main = "Dataset (a)",
  xlim = c(0, 1.85)
)
halfnorm(sqrt(cooks.distance(lm(y2 ~ x))),
  ylab = ("square root Cook's D"), cex = 1, main = "Dataset (b)",
  xlim = c(0, 1.85)
)
halfnorm(sqrt(cooks.distance(lm(y ~ x2))),
  ylab = ("square root Cook's D"), cex = 1, main = "Dataset (c)",
  xlim = c(0, 1.85)
)
graphics.off()
...
## Figures 10.5 - 18.9 ----
dat <- read.table(file = "WeekIntAllData.txt", header = TRUE)
attach(dat)
cm10_dif <- diff(cm10)
aaa_dif <- diff(aaa)
cm30_dif <- diff(cm30)
...
n <- length(cm10)
pdf("WeeklyInterestCookD.pdf", width = 6, height = 5)
par(mfrow = c(2, 2))
plot(hatvalues(fit),
  ylab = "Leverage", xlab = "Index",
  main = "(a)"
)
plot(2:n, rstudent(fit),
  ylab = "rstudent", xlab = "Index",
  main = "(b)"
)
plot(2:n, cooks.distance(fit), ylab = "Cook's D", xlab = "Index", main = "(c)")
plot(2:n, cooks.distance(fit),
  ylab = "Cook's D",
...
pdf("ResidEx5.pdf", height = 4, width = 7)
par(mfrow = c(1, 2))
...
pdf("ResidEx1.pdf", height = 4, width = 7)
par(mfrow = c(1, 2))
...
pdf("ResidEx3.pdf", height = 2.2, width = 5)
par(mfrow = c(1, 3))
...
pdf("ResidEx6.pdf", height = 5.5, width = 6)
par(mfrow = c(2, 2))
