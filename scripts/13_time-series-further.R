...
par(mfrow = c(1, 1))
plot(year[t1], x[t1],
  ylim = c(8.25, 10.3), xlim = c(1992.50, 2006.00), type = "b", lty = 2,
...
dwtest(fit, alternative = "two.sided", exact = FALSE)
dwtest(fit, alternative = "two.sided", exact = TRUE)
resid <- residuals(fit)
n <- length(resid)
tt <- (1:n) / (n + 1)
...
qqnorm(resid,
  datax = T, main = "(a) Normal plot", xlab = "theoretical quantiles",
  ylab = "sample quantiles"
)
qqline(resid, datax = TRUE)
qqplot(resid, (-0.00035 + 0.04058 * qt(tt, df = 3)),
  main = "(b) t-plot",
...
sqrt(diag(NeweyWest(fit, lag = 0, prewhite = FALSE)))
sqrt(diag(NeweyWest(fit, lag = 3, prewhite = FALSE)))

coef(fit) / sqrt(diag(NeweyWest(fit, lag = 0, prewhite = FALSE)))
coef(fit) / sqrt(diag(NeweyWest(fit, lag = 3, prewhite = FALSE)))
...
pdf("ice_cream_tsplots.pdf", height = 5, width = 6)
...
plot(0:10, forecasts[2, ],
  type = "b", lty = 2, ylim = c(0, 0.006), pch = "*", cex = 3,
...
abline(h = means[1])
abline(h = means[2], lty = 2)
legend(2, 0.0015, c("IP", "CPI"),
  lty = c(2, 1), pch = c("*", "o"), cex = 1.6, lwd = 3,
  pt.cex = c(3, 2)
)
...
mean_CPI <- ul_CPI
for (k in 1:11) {
  ul_CPI[k] <- quantile(fore_CPI[, k], 0.975)
  ll_CPI[k] <- quantile(fore_CPI[, k], 0.025)
...
mean_IP <- ul_IP
for (k in 1:11) {
  ul_IP[k] <- quantile(fore_IP[, k], 0.975)
  ll_IP[k] <- quantile(fore_IP[, k], 0.025)
...
par(mfrow = c(2, 1))
plot(0:10, ul_CPI,
  type = "b", lty = 2, ylim = c(-0.003, 0.013), lwd = 2,
...
plot(0:10, ul_IP,
  type = "b", lty = 2, ylim = c(-0.02, 0.025), lwd = 2,
  xlab = "h", ylab = "forecast", cex.axis = 1.5, cex.lab = 1.5,
...
par(mfrow = c(1, 2))
acf(diffseries(x, 0.7), main = expression(paste(Delta^0.7, Y)))
...
var1 <- ar(del_dat, order.max = 4, aic = TRUE)
var1
