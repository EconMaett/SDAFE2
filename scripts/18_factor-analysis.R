...
plot(time, as.vector(dat[1, 2:12]),
  ylim = c(0, 6), type = "b", lty = 1, lwd = 2,
...
plot(pca, main = "(b)")
plot(time, pca$rotation[, 1],
  ylim = c(-.8, .8), type = "b", lwd = 2, ylab = "PC", xlab = "T",
  main = "(c)"
)
...
par(mfrow = c(2, 2))
plot(time, mu,
  ylim = c(2.75, 4.65), type = "b", lwd = 4, xlab = "T", ylab = "Yield",
...
legend("bottomright", c(
  "mean", "mean + PC1",
  "mean -  PC1"
), lty = c(1, 5, 5), lwd = c(4, 2, 2), col = c("black", "red", "blue"))
plot(time, mu,
  ylim = c(2.75, 4.65), type = "b", lwd = 4, xlab = "T", ylab = "Yield",
  main = "(b)", xlim = c(0, 7)
)
...
legend("bottomright", c("mean", "mean + PC2", "mean -  PC2"),
  lty = c(1, 5, 5),
...
par(lwd = 1)
plot(time, pca$rotation[, 4],
  ylim = c(-.7, .7), type = "b", lwd = 2, ylab = "PC", xlab = "T",
  xlim = c(0, 30), main = "(d)"
)
...
pdf("yields_pca_tsplots.pdf", width = 7, height = 3)
par(mfrow = c(1, 3))
for (i in 1:3) {
  plot(pca$x[, i],
    main = paste("PC", toString(i)), xlab = "day",
    ylab = ""
  )
}
graphics.off()

pdf("yields_pca_acfplots2.pdf", width = 5, height = 5)
acf(pca$x[, 1:3], ylab = "", xlab = "lag")
graphics.off()

## Example 18.3 and Figure 18.5 ----
...
pdf("equityFunds_scree.pdf", width = 7.2, height = 3.5)
par(mfrow = c(1, 2))
plot(pcaEq, main = "(a)")
...
## Examples 18.6 and 18.7 and Figures 18.7 and 18.8 ----
...
pdf("FamaFrenchResidualsPairs.pdf", width = 6, height = 5)
pairs(fit$residuals)
graphics.off()
...
## Example 18.8 and Figures 18.9 and 18.10 ----
berndtInvest <- read.csv("berndtInvest.csv")
...
FF_data <- FF_data[-1, ] # delete first row since stocks_diff lost a row due to differencing
