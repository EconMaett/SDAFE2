...
plot(rf_lag, diffrf, ylab = "change in return", xlab = "return", main = "(c)", type = "p", cex = 0.7)
lines(ll_mu$x, ll_mu$y, lwd = 4, col = "red")
plot(rf_lag, (diffrf - muhat)^2, xlab = "return", ylab = "squared residual", main = "(d)", cex = 0.7)
...
scale <- 0.25
x <- seq(0, 1, length = 75)
true <- 0.1 * x + sin(5 * x^1.5) + 3.6
y <- true + rnorm(75, sd = 0.2)
h <- dpill(x, y)
...
pdf("loclin.pdf", width = 6, height = 4.5)
par(mfrow = c(1, 1))
...
pdf("loclinVaryBand.pdf", width = 6, height = 5)
par(mfrow = c(1, 1))
plot(x, y, ylim = c(2.4, 5), cex = 0.5, pch = "o")
lines(fit1$x, fit1$y, lwd = 4)
lines(fit2$x, fit2$y, lty = 2, lwd = 4, col = "red")
lines(fit3$x, fit3$y, lty = 4, lwd = 4, col = "blue")
legend("bottomleft", c("dpi", "3*dpi", "dpi/3"),
  lty = c(1, 2, 1), lwd = 4,
...
par(mfrow = c(1, 1))
plot(fit_loc_Int_F,
  ylim = c(0.025, 0.075), ylab = "Forward rate",
  col = "red", lwd = 3
)
...
legend("topleft", c("plus fn.", "derivative"), lty = c(1, 2), col = c("black", "red"), lwd = 2)
graphics.off()
...
## Example 21.2 and Figure 21.7 and 21.8 ----
library(Ecdat)
library(KernSmooth)
library(locfit)
library(mgcv)
...
orrf <- order(rf_lag)
pdf("riskfree02.pdf", width = 8, height = 4)

par(mfrow = c(1, 2))
plot(rf_lag[orrf], gam_mu$fit[orrf],
  type = "l", lwd = 3, lty = 1,
  xlab = "lagged rate", ylab = "change in rate", main = "(a)"
)
lines(ll_mu$x, ll_mu$y, lwd = 3, lty = 2, col = "red")
lines(locfit_mu, lwd = 3, lty = 3, col = "blue")
legend(0.1, -.05, c("spline", "local linear", "local quadratic"),
  lty = c(1, 2, 3), cex = 0.85, lwd = 3, col = c("black", "red", "blue")
)
rug(rf_lag)
abline(h = 0, lwd = 2)
plot(rf_lag[orrf], gam_sig$fit[orrf],
  type = "l", lwd = 3, lty = 1, ylim = c(0, 0.03),
...
pdf("riskfree04.pdf", width = 7, height = 3)
par(mfrow = c(1, 3))
...
pdf("forward_spline.pdf", width = 6, height = 5)
par(mfrow = c(1, 1))
...
par(mfrow = c(1, 3))
plot(t, X3 %*% param4[1:m2],
  ylim = c(0, 16), ylab = "rate",
  main = "(a)", col = "red", type = "l", lwd = 2
)
lines(t, lag_r1)
legend("topleft", c("theta(t)", "lagged rate"),
  lwd = c(2, 1),
  col = c("red", "black")
)

plot(t, X3[, 1:2] %*% param4[(m2 + 1):(m2 + 2)],
  ylab = "a(t)",
