...
ind2 <- (means > means[ind])
plot(risk, means,
  type = "l", lwd = 1, xlim = c(0, 0.3), ylim = c(0.0575, 0.18),
  main = paste("rho = ", rho)
)
abline(v = 0)
lines(risk[ind2], means[ind2], type = "l", lwd = 5, col = "red")
...
muP <- seq(0.05, 0.14, length = 300) # set of 300 possible target values for the expect portfolio return
...
ind3 <- (muP > muP[ind2])
lines(sdP[ind3], muP[ind3],
  type = "l", xlim = c(0, 0.25),
...
# set of 300 possible target values for the expect portfolio return
...
#  plot the efficient frontier (and inefficient frontier)
plot(sdP, muP, type = "l", xlim = c(0, 2.5), ylim = c(0, 0.15), lty = 3) 
mufree <- 1.3 / 253 # input value of risk-free interest rate
points(0, mufree, cex = 4, pch = "*") # show risk-free asset
...
## Examples 16.8 and 16.9 and Figure 16.5 ----
library(quadprog)
dat <- read.table("countries.txt", header = TRUE)
dat <- dat[, 4:13]
n <- dim(dat)[1]
N <- dim(dat)[2]
...
par(mfrow = c(1, 2))
boxplot(out_Short ~ gp, main = "(a) Short Sales Allowed", ylim = c(0, 0.7))
abline(h= 0.3681, lwd = 3, lty = 2)
...
out_NoShort <- out
boxplot(out_NoShort ~ gp, main = "(b) No Short Sales", ylim = c(0, 0.7)) 
abline(h= 0.3503, lwd = 3, lty = 2)
graphics.off()


## Table 16.1 ----
# Because of Monte Carlo variability, the results in the table will not be reproduced exactly.  
# Increasing nboot will reduce Monte Carlo variability
nboot <- 50000
library(bootstrap)
for (i in 1:10) {
...
pdf("countries_boot_shrinkNoShrink.pdf", width = 7, height = 4) 
par(mfrow = c(1, 2))
boxplot(out_NoShort ~ gp, main = "(a) No Shrinkage", ylim = c(0, 0.7))
abline(h= 0.3503, lwd = 3, lty = 2)
boxplot(out_NoShortShrink ~ gp, main = "(b) Shrinkage", ylim = c(0, 0.7))
abline(h= 0.3503, lwd = 3, lty = 2)
graphics.off()
...
for (i in 1:nlambda) {
  ...
  opt <- solve.QP(
    Dmat = as.matrix(lambda^2 * cov_mat), dvec = lambda * mean_vect,
...
par(mfrow = c(1, 3))
plot(loglambda_vect, mu_vect,
  type = "l", lwd = 2,
  xlab = expression(paste("log(", lambda, ")")), ylab = "E(return)"
)
plot(loglambda_vect, sd_vect,
  type = "l", lwd = 2,
  xlab = expression(paste("log(", lambda, ")")), ylab = "SD(return)"
)
plot(sd_vect, mu_vect,
  type = "l", lwd = 2, xlab = "SD(return)",
...
prices <- cbind(
  dat$GM_AC, dat$F_AC, dat$CAT_AC, dat$UTX_AC,
...
const.dir <- c(rep("<=", 2 * M), "=")
resultLP_min <- solveLP(
  cvec = cLP, bvec = bvecLP, Amat = AmatLP,
  lpSolve = T, const.dir = const.dir, maximum = FALSE
)
resultLP_max <- solveLP(
  cvec = cLP, bvec = bvecLP,
