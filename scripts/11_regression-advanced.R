...
legend("topright", c("price", "predicted price"),
  pch = c("*", NA),
...
pdf("DefaultProbResid1.pdf", width = 7, height = 3.5)
par(mfrow = c(1, 2))
fitted_nls <- -sum_nls$resid + freq2
plot(fitted_nls, abs(sum_nls$resid),
  xlab = "fitted values",
...
qqnorm(sum_nls$resid,
  datax = TRUE, main = "", ylab = "sample quantiles",
  xlab = "theoretical quantiles"
)
qqline(sum_nls$resid, datax = TRUE)
graphics.off()
...
fitQuad <- nls(price ~ 100 * exp(-beta0 * T - (beta1 * T^2) / 2 - (beta2 * T^3) / 3),
  data = dat,
  start = list(beta0 = 0.03, beta1 = 0, beta2 = 0)
)
coefQuad <- summary(fitQuad)$coef[, 1]
forwardQuad <- coefQuad[1] + (coefQuad[2] * t) + (coefQuad[3] * t^2)

fitCubic <- nls(price ~ 100 * exp(-beta0 * T - (beta1 * T^2) / 2 - (beta2 * T^3) / 3 - (beta3 * T^4) / 4),
  data = dat,
...
fitSpline <- nls(
  price ~ 100 * exp(-beta0 * T - (beta1 * T^2) / 2 - (beta2 * T^3) / 3
    - (T > 15) * (beta3 * (T - 15)^3) / 3),
  data = dat,
  start = list(beta0 = 0.047, beta1 = 0.0024, beta2 = 0, beta3 = -0.00007)
)
...
pdf("empirical.pdf", width = 8, height = 4.5) 
par(mfrow = c(1, 2))
plot(dat$T, dat$price, xlab = "maturity", ylab = "price", main = "(a)", cex = 0.75)
plot(dat$T[2:length(dat$T)], emp,
  ylim = c(0.025, .075), lwd = 2, xlab = "maturity",
  ylab = "empirical forward rate", type = "b", cex = 0.75,
  main = "(b)"
)
graphics.off()


pdf("stripsNL01.pdf", width = 6, height = 5)
par(mfrow = c(1, 1))
...
pdf("DefaultProbResid2.pdf", width = 7, height = 3.5)
par(mfrow = c(1, 2))
...
## Example 11.6 and figures 11.7 and 11.8 ----
n <- 80
set.seed("781235")
...
pdf("simDataBoxCox.pdf", width = 6, height = 5)
library(MASS)
par(mfrow = c(1, 1))
...
pdf("simDataBoxCoxResid.pdf", width = 6, height = 6)
par(mfrow = c(2, 2))
...
qqnorm(rstudent,
  datax = TRUE, main = "(d)", xlab = "theoretical quantiles",
  ylab = "sample quantiles"
)
qqline(rstudent, datax = TRUE)
graphics.off()


## Example 11.7 and figures 11.9 and 11.10 ----
library("AER")
library("faraway")
library(MASS)
data("CreditCard")
...
pdf("CreditCardHist.pdf", width = 5.5, height = 5.5)
par(mfrow = c(3, 3))
...
pdf("CreditCards_effects.pdf", width = 6, height = 5)
par(mfrow = c(2, 2))
plot(reports_grid, plogis(9.5238 - 2.8953 * reports_grid2),
  type = "b",
  lwd = 2, xlab = "reports", ylab = "P(accept)", ylim = c(0, 1)
)
plot(income_grid, plogis(9.5238 + 0.8717 * (income_grid - mean(income))),
  type = "l",
  cex = 2, lwd = 2, xlab = "income", ylab = "P(accept)", ylim = c(0, 1)
)
plot(log(share_grid), plogis(9.5238 + 3.3102 * share_grid2),
  type = "l",
  cex = 2, lwd = 2, xlab = "log(share)", ylab = "P(accept)", ylim = c(0, 1)
)
plot(0:6, plogis(9.5238 - .5506 * ((0:6) - mean(dependents))),
  type = "b",
...
pdf("CreditCards_share_card.pdf", width = 6, height = 5)
par(mfrow = c(2, 2))
...
## Figure 11.12 and example 11.8 ----
x <- seq(-1, 2.5, .025)
n <- length(x)
set.seed(3592)
y <- exp(-x) + rnorm(n, sd = 0.06)
pdf("linearizing_sim.pdf", width = 6, height = 5)
par(mfrow = c(2, 2), cex.axis = 1.05, cex.lab = 1.05)
plot(x, y, main = "(a)")
plot(x, log(y), main = "(b)")
fit <- lm(log(y) ~ x)
qqnorm(fit$res, datax = TRUE, main = "(c)", ylab = "sample quantiles", xlab = "theoretical
       quantiles")
qqline(fit$res, datax = TRUE)
...
legend("bottomright", c(
  "good data", "residual outliers", "with outliers",
  "w/o outliers"
),
...
plot(lag_r1,
  ylim = c(0, 16), ylab = "rate and theta",
  main = "(a)", type = "l"
)
