...
## Figure 7.1 ----
pdf("CRSPday_scatterplot.pdf", width = 7, height = 7)
data(CRSPday, package = "Ecdat")
...
## Figure 7.2 ----
pdf("bivariate_normal_contours.pdf", width = 8, height = 4.5)
library("mnormt")
mu <- rep(0, 2)
...
t <- seq(-2.5, 2.5, .025)
ff1 <- function(x, x2) {
  dmnorm(cbind(x, x2), mu, cov1)
}
ff2 <- function(x, x2) {
  dmnorm(cbind(x, x2), mu, cov2)
}
par(mfrow = c(1, 2))
contour(t, t, outer(t, t, ff1),
  xlab = expression(Y[1]), ylab = expression(Y[2]),
  cex.lab = 1, cex.axis = 1, labcex = .8, main = "(a) corr = 0.5"
)
contour(t, t, outer(t, t, ff2),
  xlab = expression(Y[1]), ylab = expression(Y[2]),
...
## Figure 7.3 ----
pdf("MultiT_IndT.pdf", width = 8, height = 4.5)
library(sn)
n <- 2500
df <- 3
set.seed(5600)
...
par(mfrow = c(1, 2))
plot(x[, 1], x[, 2],
  main = "(a) Multivariate-t",
  xlab = expression(Y[1]), ylab = expression(Y[2])
)
abline(h = 0)
abline(v = 0)
plot(y1, y2,
  main = "(b) Independent t",
  xlab = expression(Y[1]), ylab = expression(Y[2])
)
abline(h = 0)
abline(v = 0)
...
## Figure 7.4 ----
pdf("midcapDPairs.pdf", width = 6, height = 6)
...
## Example 7.4 and Figure 7.5 ----
pdf("CRSPday_MultiT_profile.pdf")
library(mnormt)
library(MASS)
data(CRSPday, package = "Ecdat")
dat <- CRSPday[, 4:7]
...
## Section 7.8 including Figure 7.6 ----
...
## Figure 7.7 ----
pdf("bivariate_skew_t.pdf", width = 5, height = 5)
library(sn)
t <- seq(-2.5, 2.5, .025)
dp <- list("xi" = c(0, 0), "Omega" = diag(c(1, 1)), "alpha" = c(-1, .25), "nu" = 4)
ff3 <- function(x, x2) {
  dmst(cbind(x, x2), dp = dp)
}
contour(t, t, outer(t, t, ff3),
  xlab = expression(X[1]), ylab = expression(X[2]),
  cex.lab = 1, cex.axis = 1, labcex = 1, main = expression(paste(
...
## Figure 7.8 -----
pdf("CRSPNormalPlots.pdf", width = 6, height = 5)
data(CRSPday, package = "Ecdat")
dat <- CRSPday[, 4:7]
par(mfrow = c(2, 2))
...
## Example 7.7 including Figure 7.9 ----
...
for (i in 1:4) {
  print(quantile(results[, i], c(.025, .975)))
}
for (i in 1:4) {
...
for (i in 1:n) {
  ...
  loglik_profile[i] <- sum(log(dmt(Berndt,
    mean = fit$center,
    S = fit$cov, df = df[i]
  )))
}
...
fit_mvt <- optim(start, loglik,
  method = "L-BFGS-B",
  lower = c(-.02, -.02, -.1, -.1, -.1, 2), upper = c(.02, .02, .1, .1, .1, 15), hessian = TRUE
)
