...
pdf("skewed_densities.pdf")
par(cex.axis = 1.5, cex.lab = 1.5, mfrow = c(2, 2))
plot(x - 4 / 14, dbeta(x, 4, 10),
  type = "l", ylab = "Density(y)", main = "(a): Right-skewed",
  xlab = "y"
)
abline(v = 0)
plot(x - 10 / 14, dbeta(x, 10, 4),
  type = "l", ylab = "Density(y)", main = "(b): Left-skewed",
  xlab = "y"
)
abline(v = 0)
plot(x - 1 / 2, dbeta(x, 7, 7),
  type = "l", ylab = "Density(y)", main = "(c): Symmetric",
  xlab = "y"
)
abline(v = 0)
graphics.off()
...
## Figures 5.2, 5.3, and 5.4 were created by MATLAB

## Figure 5.5  ----
pdf("NormMix01.pdf", height = 7, width = 6)
par(mfrow = c(2, 2))
x <- seq(-6, 15, by = .01)
plot(x, dnorm(x, sd = sqrt(3.4)),
  type = "l", lwd = 2, ylim = c(0, 0.4),
  xlab = "x", ylab = "density", main = "(a) densities"
)
lines(x, 0.9 * dnorm(x) + .1 * dnorm(x, sd = 5), lwd = 2, lty = 5, col = "red")
legend("topright", c("normal", "mixture"), lwd = 2, lty = c(1, 5), col = c("black", "red"))

plot(x, dnorm(x, sd = sqrt(3.4)),
  type = "l", lwd = 2, ylim = c(0, 0.025), xlim = c(4, 15),
...
qqnorm(y1,
  datax = TRUE, main = "(c) QQ plot, normal", xlab = "theoretical quantiles",
  ylab = "sample quantiles"
)
qqline(y1, datax = TRUE, col = "red")
n2 <- rbinom(1, 200, 0.9)
y2 <- c(rnorm(n2), rnorm(200 - n2, sd = 5))
qqnorm(y2,
  datax = TRUE, main = "(d) QQ plot, mixture", xlab = "theoretical quantiles",
  ylab = "sample quantiles"
)
qqline(y2, datax = TRUE, col = "red")
graphics.off()


## Figure 5.6 ----
pdf("tailweights.pdf", height = 6, width = 6)
par(mfrow = c(1, 1))
library(fGarch)
x <- seq(from = 1, to = 100, by = .01)
plot(x, dged(x, nu = 1.5),
  log = "xy", type = "l", lwd = 2,
...
## Figure 5.7 ----
pdf("GedStPeaks.pdf", height = 5, width = 6)
...
x2 <- seq(0, 5, 0.01)
x3 <- seq(-5, 0, 0.01)
plot(x, dstd(x, nu = 10), type = "l", lwd = 3, ylim = c(0, 0.6), ylab = "density")
...
loglik_t <- function(beta) {
  sum(-dt((x - beta[1]) / beta[2],
    beta[3],
    log = TRUE
  ) + log(beta[2]))
}
fit_t <- optim(start, loglik_t,
...
fit_sstd <- optim(start, loglik_sstd,
  hessian = TRUE, method = "L-BFGS-B",
  lower = c(-0.1, 0.01, 2.1, -2)
)
...
fit_ged <- optim(start, loglik_ged,
  hessian = TRUE, method = "L-BFGS-B",
  lower = c(-0.1, 0.01, 1)
)
...
diffrf <- diff(Capm$rf)
par(mfrow = c(1, 1))
plot(density(diffrf, kernel = "gaussian", adjust = 1),
  xlim = c(-0.3, 0.3),
...
## Example 5.6 ----
pdf("FlowSkewTAzzalini.pdf", width = 6, height = 6)
library(sn)
...
legend(40, 0.034, c("t-model", "KDE"), lty = c(1, 2), lwd = c(2, 2), cex = 1, col = c("black", "red"))
n <- length(x1)
u <- (1:n) / (n + 1)
...
plot(x2, dsn(x2, dp = est2),
  type = "l", lwd = 2, xlab = "flow", ylab = "flow 2 density", ylim = c(0, 0.055)
)
d <- density(x2)
lines(d$x, d$y, lty = 2, lwd = 2, col = "red")
legend(162, 0.051, c("t-model", "KDE"), lty = c(1, 2), lwd = c(2, 2), cex = 1, col = c("black", "red"))
...
plot(x3, dsn(x3, dp = est3),
  type = "l", lwd = 2, xlab = "flow", ylab = "flow 3 density", ylim = c(0, 0.056)
)
d <- density(x3)
lines(d$x, d$y, lty = 2, lwd = 2, col = "red")
legend(3, 0.0575, c("t-model", "KDE"), lty = c(1, 2), lwd = c(2, 2), cex = 1, col = c("black", "red"))
...
qqnorm((x2^10.5 - 1) / 10.5, datax = TRUE, main = "Flow 2")
x <- dat$Flow3
x3 <- sort(x)
bcfit3 <- boxcox(x3 ~ 1, lambda = seq(0.6, 2, 1 / 100), xlab = expression(alpha))
text(1.6, -1793, "Flow 3")
plot(density((x3^1.235 - 1) / 1.235, adjust = adj), main = "Flow 3")
qqnorm((x3^1.235 - 1) / 1.235, datax = TRUE, main = "Flow 3")
graphics.off()


## Figure 5.13 - 5.16 ----
pdf("diffrf_TKDE.pdf", width = 6, height = 5)
data(Capm, package = "Ecdat")
y <- diff(Capm$rf)
diffrf <- y
library(fGarch)
...
pdf("diffrf_TKDE_detail.pdf", width = 6, height = 5)
d1 <- density(diffrf)
plot(d1$x, d1$y,
  type = "l", xlab = "y", ylab = "Density(y)", xlim = c(-0.5, -0.1),
  ylim = c(0, 1), lwd = 2
)
d2 <- density(x)
ginvx <- qstd(pnorm(d2$x), mean = 0.001, sd = .0725, nu = 3.34)
...
pdf("diffrf_TKDE_trans.pdf", width = 6, height = 5)
ygrid <- seq(min(y), max(y), 0.01)
plot(ygrid, qnorm(pstd(ygrid, mean = 0.001, sd = .0725, nu = 3.34)),
  type = "l", lwd = 2, xlab = "y", ylab = "g(y)"
)
graphics.off()

pdf("diffrf_TKDE_QQ.pdf", width = 6, height = 5)
...
bc <- boxcox(male.earnings ~ 1, lambda = seq(0.3, 0.45, by = 1 / 100), interp = F)
ind <- (bc$y == max(bc$y))
...
Y <- diff(log(EuStockMarkets[, 1])) #  DAX
X <- qnorm(pstd(Y,
  mean = fit_std$par[1], sd = fit_std$par[2],
  nu = fit_std$par[3]
)) #  transformed data
d2 <- density(X) # KDE of transformed data
ginvx <- qstd(pnorm(d2$x),
  mean = fit_std$par[1],
  sd = fit_std$par[2], nu = fit_std$par[3]
) # inverse transformation
gprime_num <- dstd(ginvx,
  mean = fit_std$par[1],
  sd = fit_std$par[2],
  nu = fit_std$par[3]
) # numerator of derivative of transformation
gprime_den <- dnorm(qnorm(pstd(ginvx,
  mean = fit_std$par[1],
  sd = fit_std$par[2],
  nu = fit_std$par[3]
))) # denominator of derivative of transformation
...
plot(d1$x, d1$y,
  type = "l", xlab = "y", ylab = "Density(y)", xlim = c(0.035, 0.06),
  ylim = c(0, 0.8), lwd = 2
) # plot KDE of untransformed data, only right tail
x <- seq(-0.09, 0.09, by = 0.001)
lines(x, dstd(
  x, fit_std$par[1], fit_std$par[2],
  fit_std$par[3]
), lty = 3, lwd = 2) # plot parametric fit
...
fit_sstd <- optim(start, loglik_sstd,
  method = "L-BFGS-B",
  lower = c(-0.1, 0.001, 2.1, 0.1),
  upper = c(0.1, 1, 20, 10), hessian = TRUE
)
...
loglik <- function(theta) {
  -sum(log(dpois(y, lambda = exp(theta[1] +
    theta[2] * x))))
}
