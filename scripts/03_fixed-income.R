...
# Computes bv = bond values (current prices) corresponding to all values 
# of yield to maturity in the input vector r
# INPUT
...
# Computes the yield to maturity of a bond paying semi-annual coupon payments
...
plot(r, value,
  xlab = "yield to maturity", ylab = "price of bond", type = "l",
...
## Figure 3.2 ----
rates <- as.matrix(read.table("WeeklyInterest.txt", header = FALSE))
ff <- rates[, 4] / 100
tb03 <- rates[, 5] / 100
cm10 <- rates[, 6] / 100
cm30 <- rates[, 7] / 100
...
legend("topright", c("3-month", "10-year", "30-year"),
  lty = c(2, 1, 3), lwd = c(3, 2, 4),
...
### Section 3.10.2 ----
mk.maturity <- read.csv("mk.maturity.csv", header = TRUE)
mk.zero2 <- read.csv("mk.zero2.csv", header = TRUE)
plot(mk.maturity[, 1], mk.zero2[5, 2:56],
  type = "l",
...
### Exercise 2 ----
forward <- function(t) {
  0.04 + 0.0002 * t - 0.00003 * t^2
}
yield <- function(t) {
  0.04 + 0.0001 * t - 0.00001 * t^2
}
...
C <- 25
price <- 1015
yieldToPrice <- function(C, par, yield, T) {
  sum(C / (1 + yield)^(1:(2 * T))) + par / (1 + yield)^(2 * T)
}
yield <- uniroot(function(yield) {
  yieldToPrice(C, par, yield, T) - price
}, lower = 0.0001, upper = 0.1)
round(yield$root, 4)
