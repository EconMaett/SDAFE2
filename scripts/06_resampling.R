...
rowMeans(results$thetastar[, ]) # Table 6.1, row 2
apply(results$thetastar[, ], 1, sd) # Table 6.1, row 4
fitdistr(ge, "t") # Table 6.1, rows 1 and 3
apply(results_250$thetastar, 1, mean) # Table 6.2, row 2
apply(results_250$thetastar, 1, sd) # Table 6.2, row 4
fitdistr(ge_250, "t") # Table 6.2, rows 1 and 3
quantile(results_250$thetastar[3, ], c(.95, .98, .99, .999))
pdf("MLE_t_BS_250.pdf", width = 7, height = 3.5) # Figure 6.1
par(mfrow = c(1, 2))
plot(density(results_250$thetastar[3, ]),
  xlab = "df",
  xlim = c(2, 21), main = "(a) n = 250"
)
plot(density(results$thetastar[3, ]),
  xlab = "df",
...
## Figure 6.2 and example 6.4 ----
pdf("LSCC_CSGSQQ.pdf", width = 6, height = 5)
par(mfrow = c(1, 1))
...
q.grid <- (1:n) / (n + 1)
qqplot(bmwRet[, 2], qsstd(
  q.grid, fit_skewt$estimate[1],
...
for (i in 1:nboot) {
  ...
  samp_ModelBased <- rsstd(
    n, fit_skewt$estimate[1],
    fit_skewt$estimate[2],
    fit_skewt$estimate[3], fit_skewt$estimate[4]
  )
