...
plot(rmrf2, rfood2,
  ylab = "Food industry excess return",
...
plot(1:3, b$bic,
  type = "b", xlab = "number of variables",
  ylab = "BIC", cex = 2.5
)
plot(1:3, b$cp,
  type = "b", xlab = "number of variables",
...
summary(lm(step_lm))
x1 <- as.matrix(cbind(
  diff(gnp.r), diff(gnp.pc), diff(log(ip)),
  diff(log(cpi)),
  diff(emp), diff(bnd)
))
names_x1 <- c(
  "gnp.r", "gnp.pc", "log(ip)", "log(cpi)",
  "emp", "bnd"
)
...
pdf("weekint_partialresidual.pdf", width = 6, height = 5)
par(mfrow = c(2, 2))
crPlot(fit2, var = "cm10_dif", main = "(a)", smooth = FALSE, lty = 1, lwd = 2, col = "black")
crPlot(fit2, var = "cm30_dif", main = "(b)", smooth = FALSE, lty = 1, lwd = 2, col = "black")
...
step_lm <- stepAIC(fit_lm)
fit_lm2 <- lm(step_lm)
pdf("nelsonPlosser_partResid.pdf", width = 6, height = 5)
par(mfrow = c(2, 2))
library("car")
crPlot(fit_lm2, var = "diff(gnp.r)", main = "(a)", smooth = FALSE, lty = 1, lwd = 2)
crPlot(fit_lm2, var = "diff(gnp.pc)", main = "(b)", smooth = FALSE, lty = 1, lwd = 2)
crPlot(fit_lm2, var = "diff(log(ip))", main = "(c)", smooth = FALSE, lty = 1, lwd = 2)
crPlot(fit_lm2, var = "diff(bnd)", main = "(c)", smooth = FALSE, lty = 1, lwd = 2)
graphics.off()
